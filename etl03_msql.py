...
try:
    ...
    with open('/Users/eduardoaandrad/Dropbox/Desenv/python/3_analise/infopreco.csv', encoding = "ISO-8859-1") as arq:
        ...
        csv_data
        for row in csv_data:
            cur.execute('insert into mydesenv.t_tmp_valores(CNPJ, NOME, ENDERECO, COMPLEMENTO, BAIRRO, MUNICIPIO, UF, PRODUTO, VALOR_VENDA, DATA_CADASTRO) values(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)',row[:55])
            mydb.commit()
except IOError:
